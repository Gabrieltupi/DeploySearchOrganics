...
CREATE SEQUENCE seq_pedidoxprodutos
    START WITH 1
    INCREMENT BY 1
    NOCACHE
    NOCYCLE;
...
(
    ...
    setor             VARCHAR2(100),
    CONSTRAINT FK_EMPRESA_USUARIO_ID
        FOREIGN KEY (id_usuario)
            REFERENCES Usuario (id_usuario)
);
...
(
    ...
    CONSTRAINT FK_ORDEM_PEDIDO_PEDIDO_ID
...
ALTER TABLE OrdemPedido
    ADD CONSTRAINT FK_ORDEM_PEDIDO_PEDIDO_ID FOREIGN KEY (id_pedido) REFERENCES Pedido (id_pedido);

ALTER TABLE PedidoxProduto
    ADD CONSTRAINT FK_PEDIDO_PRODUTOS_PEDIDO_ID FOREIGN KEY (id_pedido) REFERENCES Pedido (id_pedido);

ALTER TABLE PedidoxProduto
    ADD CONSTRAINT FK_PEDIDO_PRODUTOS_PRODUTO_ID FOREIGN KEY (id_produto) REFERENCES Produto (id_produto);

ALTER TABLE PedidoXProduto
    ADD CONSTRAINT PK_PEDIDO_PRODUTO PRIMARY KEY (id_pedido, id_produto);

ALTER TABLE Pedido
    ADD CONSTRAINT FK_PEDIDO_CUPOM_ID
        FOREIGN KEY (id_cupom)
            REFERENCES Cupom (id_cupom);

ALTER TABLE Pedido
    ADD CONSTRAINT FK_PEDIDO_USUARIO_ID
        FOREIGN KEY (id_usuario)
            REFERENCES Usuario (id_usuario);

ALTER TABLE Pedido
    ADD CONSTRAINT FK_PEDIDO_ENDERECO_ID FOREIGN KEY (id_endereco) REFERENCES Endereco (id_endereco);